...
void OutputHandler::ChangeOutput(uint16_t message)
{
...
uint8_t OutputHandler::_toggleSelectedOutput(uint8_t outputNumber)
{
	/*
	switch(outputNumber)
	{
	case 0: 
		...
		settingsData.IsEnabled(outputNumber) ? _output_0.ToggleLed() : _output_0.TurnOffLed();
		break;
	case 1: 
		_output_0.ToggleRelay();
		settingsData.IsEnabled(outputNumber) ? _output_0.ToggleLed() : _output_0.TurnOffLed();
